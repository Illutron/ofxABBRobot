...
void ofSerialEven::enumerateWin32Ports(){
...
//----------------------------------------------------------------
void ofSerialEven::enumerateDevices(){
	...
	//---------------------------------------------
		...
		if (dir == NULL){
			ofLog(OF_LOG_ERROR,"ofSerialEven: error listing devices in /dev");
		} else {
			printf("ofSerialEven: listing devices\n");
			while ((entry = readdir(dir)) != NULL){
				str = (char *)entry->d_name;
				if( str.substr(0,5) == "tty.u" ){
	...
	//---------------------------------------------
		...
		enumerateWin32Ports();
		printf("ofSerialEven: listing devices (%i total)\n", nPorts);
...
//----------------------------------------------------------------
bool ofSerialEven::setup(){
	return setup(0,9600);		// the first one, at 9600 is a good choice...
}

//----------------------------------------------------------------
bool ofSerialEven::setup(int deviceNumber, int baud){
	...
	//---------------------------------------------
		...
		while ((entry = readdir(dir)) != NULL){
			...
			if( str.substr(0,4) == "ttyS" || str.substr(0,6) == "ttyUSB" || str.substr(0,3) == "rfc" ){
				...
				if(deviceCount == deviceNumber){
					device = "/dev/"+str;
					deviceFound = true;
					ofLog(OF_LOG_NOTICE,"ofSerialEven device %i - /dev/%s  <--selected", deviceCount, str.c_str());
				}else ofLog(OF_LOG_NOTICE,"ofSerialEven device %i - /dev/%s", deviceCount, str.c_str());
				deviceCount++;
			}
		}
        ...
        if(deviceFound){
            ...
            ofLog(OF_LOG_ERROR,"ofSerialEven: could not find device %i - only %i devices found", deviceNumber, deviceCount);
            return false;
        }
	...
	//---------------------------------------------
        ...
        if(deviceFound){
            ...
            ofLog(OF_LOG_ERROR,"ofSerialEven: could not find device %i - only %i devices found", deviceNumber, nPorts);
            return false;
        }
...
//----------------------------------------------------------------
bool ofSerialEven::setup(string portName, int baud){
	...
	//---------------------------------------------
	    ...
	    ofLog(OF_LOG_NOTICE,"ofSerialEvenInit: opening port %s @ %d bps", portName.c_str(), baud);
		fd = open(portName.c_str(), O_RDWR | O_NOCTTY | O_NONBLOCK);
		if(fd == -1){
			ofLog(OF_LOG_ERROR,"ofSerialEven: unable to open port");
			return false;
		}
		...
		switch(baud){
		   ...
		   case 115200: cfsetispeed(&options,B115200);
			...
			default:	cfsetispeed(&options,B9600);
						cfsetospeed(&options,B9600);
						ofLog(OF_LOG_ERROR,"ofSerialEvenInit: cannot set %i baud setting baud to 9600\n", baud);
						break;
		}
	...
	sprintf(buf,"baud=%d parity=N data=8 stop=1",bps);
		...
		if(!BuildCommDCBA(buf,&cfg.dcb)){
			ofLog(OF_LOG_ERROR,"ofSerialEven: unable to build comm dcb; (%s)",buf);
		}
	#else
		if(!BuildCommDCB(buf,&cfg.dcb)){
			ofLog(OF_LOG_ERROR,"ofSerialEven: Can't build comm dcb; %s",buf);
		}
	#endif
	...
	if(!SetCommState(hComm,&cfg.dcb)){
		ofLog(OF_LOG_ERROR,"ofSerialEven: Can't set comm state");
	}
...
//----------------------------------------------------------------
int ofSerialEven::writeBytes(unsigned char * buffer, int length){

	if (!bInited){
		ofLog(OF_LOG_ERROR,"ofSerialEven: serial not inited");
	...
	//---------------------------------------------
		...
		if(numWritten <= 0){
			if ( errno == EAGAIN )
				return 0;
			ofLog(OF_LOG_ERROR,"ofSerialEven: Can't write to com port, errno %i (%s)", errno, strerror(errno));
			return OF_SERIAL_ERROR;
		}

		ofLog(OF_LOG_VERBOSE,"ofSerialEven: numWritten %i", numWritten);
    ...
    //---------------------------------------------
		...
		if(!WriteFile(hComm, buffer, length, &written,0)){
			 ofLog(OF_LOG_ERROR,"ofSerialEven: Can't write to com port");
			 return OF_SERIAL_ERROR;
		}
		ofLog(OF_LOG_VERBOSE,"ofSerialEven: numWritten %i", (int)written);
		return (int)written;
	#endif
	//---------------------------------------------
...
//----------------------------------------------------------------
int ofSerialEven::readBytes(unsigned char * buffer, int length){
	...
	//---------------------------------------------
		...
		if(nRead < 0){
			...
			ofLog(OF_LOG_ERROR,"ofSerialEven: trouble reading from port, errno %i (%s)", errno, strerror(errno));
			return OF_SERIAL_ERROR;
		}
		return nRead;
    #endif
    ...
    //---------------------------------------------
		...
		if (!ReadFile(hComm,buffer,length,&nRead,0)){
			ofLog(OF_LOG_ERROR,"ofSerialEven: trouble reading from port");
			return OF_SERIAL_ERROR;
		}
		return (int)nRead;
	#endif
...
//----------------------------------------------------------------
bool ofSerialEven::writeByte(unsigned char singleByte){
	...
	//---------------------------------------------
		...
		ofLog(OF_LOG_VERBOSE,"ofSerialEven: written byte");
...
//----------------------------------------------------------------
void ofSerialEven::flush(bool flushIn, bool flushOut){
